...
def test_company_name_score_explicit_and_grounded_is_one():
    ...
    assert compute_scores(_input(vc))["company_name_score"] == pytest.approx(1.0)
...
def test_company_name_score_inferred_true_is_zero():
    ...
    assert compute_scores(_input(vc))["company_name_score"] == pytest.approx(0.0)
...
def test_address_score_all_five_grounded_is_one():
    ...
    assert compute_scores(_input(vc))["address_score"] == pytest.approx(1.0)


def test_address_score_all_null_is_zero():
    vc = _all_null_vendor()
    assert compute_scores(_input(vc))["address_score"] == pytest.approx(0.0)
...
def test_tax_id_score_all_grounded_is_one():
    ...
    assert compute_scores(_input(vc))["tax_id_score"] == pytest.approx(1.0)
...
def test_contact_score_all_three_grounded_is_one():
    ...
    vc["phone"] = _grounded()
    vc["email"] = _grounded()
    assert compute_scores(_input(vc))["contact_score"] == pytest.approx(1.0)
...
def test_overall_is_arithmetic_mean_of_four():
    vc = _all_null_vendor()
    # Expected scoring per FR-017: company_name contributes 1.0; address has three of five
    # slots grounded so address_score is 0.6; tax_id and contact stay at zero; the overall
    # score is the arithmetic mean of the four — (1.0 + 0.6 + 0 + 0) / 4 = 0.4.
    vc["company_name"] = _explicit_name()
...
def test_overall_all_null_is_zero():
    assert compute_scores(_input(_all_null_vendor()))["overall_vendor_identity_score"] == pytest.approx(0.0)
...
def test_overall_all_grounded_is_one():
    ...
    vc["phone"] = _grounded()
    vc["email"] = _grounded()
    assert compute_scores(_input(vc))["overall_vendor_identity_score"] == pytest.approx(1.0)
