# logo_only ground-truth reference

Use these values to author `expected.json` for each corresponding `source.pdf`. The `company_name` appears ONLY as a rasterized logo image on each PDF; all other fields below are rendered as plain text.

## logo_only_01_cascade_ironworks.pdf

- **company_name**: Cascade Ironworks
- **street_1**: 2840 Industrial Way
- **street_2**: `null`
- **city**: Reno
- **state**: NV
- **postal_code**: 89502
- **country**: US
- **ein**: 87-3319042
- **website**: invoicehub.co
- **phone**: (775) 555-0184
- **email**: billing@proton.me

## logo_only_02_ember_and_quill.pdf

- **company_name**: Ember & Quill
- **street_1**: 1144 Polk Street
- **street_2**: Suite 3B
- **city**: San Francisco
- **state**: CA
- **postal_code**: 94109
- **country**: US
- **ein**: 46-8821073
- **website**: paybill.net
- **phone**: (415) 555-0231
- **email**: accounts@gmail.com

## logo_only_03_northwind_forge.pdf

- **company_name**: Northwind Forge
- **street_1**: 4521 N Commerce Parkway
- **street_2**: `null`
- **city**: North Las Vegas
- **state**: NV
- **postal_code**: 89030
- **country**: US
- **ein**: 91-2208417
- **website**: ledgerbox.io
- **phone**: (702) 555-0148
- **email**: ar-dept@zoho.com

## logo_only_04_halcyon_provisions.pdf

- **company_name**: Halcyon Provisions
- **street_1**: 880 Oak Harbor Drive
- **street_2**: `null`
- **city**: Sacramento
- **state**: CA
- **postal_code**: 95828
- **country**: US
- **ein**: 33-5598210
- **website**: quickremit.app
- **phone**: (916) 555-0127
- **email**: billing@outlook.com

## logo_only_05_meridian_slate.pdf

- **company_name**: Meridian Slate
- **street_1**: 6710 Sierra Center Parkway
- **street_2**: Suite 120
- **city**: Reno
- **state**: NV
- **postal_code**: 89511
- **country**: US
- **ein**: 52-4407291
- **website**: sendinvoice.co
- **phone**: (775) 555-0109
- **email**: accounting@fastmail.com
