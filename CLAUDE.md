...
## Recent Changes
- 022-ocr-semantic-quality-gate (foundation slice, in-flight via stacked PRs): the foundation PR (PR #44) lands the v1.3.0 contract-set bump under `contracts/stage1_vendor_identity/v1.3.0/` with new `semantic_table_truth.schema.json` (Q-SEC-2/B safety pattern `^[A-Za-z0-9_-]{1,64}$` + `maxLength: 64` on both `document_id` and `row_id`) and the shared helpers — `src/dartwing_ocr/validator/corpus_pattern.py` (Q23 canonical-pattern allowlist `^inv_\d{3}_(easy|medium|hard)$`), `src/dartwing_ocr/evaluator/stable_json.py` (Q34 byte-identical serializer), and `SemanticGateInvariantError` (Q42/MI-19) added to the existing evaluator exception hierarchy. The actual gate (4 predicate-based checks evaluated in fixed Q17 order with no short-circuit; verdict aggregated by any-fail rule into closed snake_case status enum {`passed`, `failed`, `not_applicable`, `unevaluable`} — Q26), its synthetic CPU-only US2 fixture (PR #45), and the report surface (`semantic_table_quality` object on `evaluation_document.json` + `semantic_table_quality_metrics` namespace + `semantic_document_statuses` array on `evaluation_run_summary.json` — PR #4 / US3) land in later stacked PRs. Calibration folders are recognized via the canonical-pattern allowlist and excluded from scored aggregation (Q39/SC-009). 44 spec Clarifications + 7 security-clarify questions resolved. New FR-034 adds air-gapped operation as a named requirement (Q-SEC-7/B). Spec: `specs/022-ocr-semantic-quality-gate/spec.md`. Plan: `specs/022-ocr-semantic-quality-gate/plan.md`. Stack: PR #44 (foundation) → PR #46 (US1 validator) + PR #45 (US2 gate) → PR #4..#7 (US3..polish, not yet opened).
