...
## Recent Changes
- 022-ocr-semantic-quality-gate: Deterministic semantic table OCR quality gate landed under `src/dartwing_ocr/evaluator/` — four predicate-based checks (`malformed-currency-shape`, `missing-required-content`, `row-text-coverage-gap`, `row-alignment-failure`) evaluated in fixed Q17 order with no short-circuit and Q18 multi-category attribution; verdict aggregated by any-fail rule into closed snake_case status enum {`passed`, `failed`, `not_applicable`, `unevaluable`} (Q26). Body OCR is `preprocess_output.json` content minus the feature-020 page-1 header band (Q22), concatenated into one normalized search string (Q33 + Q5 NFKC/casefold/whitespace-collapse/Unicode-P* punctuation-strip per Q32). Currency-position token identified per declared field by digit-sequence matching on the RAW token before normalization (Q35 + Q16) and evaluated against `^\$?\d{1,3}(,\d{3})*\.\d{2}$` (Q10). Report surface: `semantic_table_quality` object (closed shape with `status`/`failed_checks`/`row_reasons`/`supporting_evidence`/optional `cause`+`cause_detail` per Q29/Q30/Q31/Q37) + `document_pass_fail.semantic_table_quality_passed: bool | null` (Q20) on `evaluation_document.json`; top-level `semantic_table_quality_metrics` namespace + per-document statuses on `evaluation_run_summary.json` (Q19/Q36). Calibration folders (Q23 canonical-pattern allowlist `^inv_\d{3}_(easy|medium|hard)$`) appear in per-document entries but are excluded from scored aggregation (Q39 + SC-009). 44 Clarifications across 4 rounds. Plan: `specs/022-ocr-semantic-quality-gate/plan.md`.
