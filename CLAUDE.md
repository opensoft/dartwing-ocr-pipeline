...
- `specs/020-vendor-evidence-gate/research.md` — 16 R-decisions covering CLI/env-var precedence (R-020.1), closed-vocabulary preset registry (R-020.2), five-signal definitions (R-020.3), regex patterns (R-020.4), coordinate filter (R-020.5), v1 decision table (R-020.6), evaluation-order (R-020.7), suppression predicate (R-020.8), `RunSummary.SCHEMA_VERSION` bump (R-020.9), run-summary field shapes (R-020.10), `document_id` semantics (R-020.11), warn-and-proceed (R-020.12), benchmark subset (R-020.13), two-metric quality gate (R-020.14), GPU deferral path (R-020.15), warm-vs-cold benchmark discipline (R-020.16)
- `specs/020-vendor-evidence-gate/data-model.md` — 10 sections: `EvidenceGate` preset, `FiveSignalSet`, `EvidenceGateResult`, `EvidenceGateDocumentRecord`, `RunSummary` additive fields, regex constants, numeric constants, stop-word set, CPU/stub identity values, state-transitions/side-effects
...
- Python 3.12 (matches `.devcontainer/Dockerfile` base image and `pyproject.toml requires-python = ">=3.12"`). + existing only — `paddleocr>=3.5,<4`, `paddlepaddle-dcu` (workstation-only optional install, already proven in features 014–017), `pypdfium2>=4.30,<5` (used for the per-page geometry read on skipped pages 2..N — no rasterization required to obtain `width_pt`/`height_pt`/`rotation`, R-018.6; also drives the page-1 header-band crop via `pypdfium2.PdfPage.render(crop=...)`, R-018.5), `Pillow>=10.4,<11` (the cropped bitmap is converted to a PIL `Image` via `bitmap.to_pil().convert("RGB")` to feed `ocr.run_page`), `numpy>=1.26,<3`, `jsonschema>=4.22,<5`, `pydantic>=2.7,<3`. **No new pinned dependency.** The `header-first-v1` preset uses `pypdfium2` page geometry plus pypdfium2's native `render(crop=...)` — no `Pillow.Image.crop` call is involved. (018-dpi-region-first-preprocess)
...
- Python 3.12 (matches `.devcontainer/Dockerfile` and `pyproject.toml requires-python = ">=3.12"`; consistent with features 001–020). + Existing only — `pytest>=8` (test framework, already declared), `PyYAML>=6` (voter-config parsing, already declared by feature 005), `paddleocr>=3.5,<4`, `paddlepaddle-dcu` (workstation-only optional install, already proven by features 014–019), `httpx>=0.27,<1` (used by feature 005 extractor, available to tests). **No new pinned dependency.** The Ollama readiness shell helper uses `curl` + `jq` + `yq` (system tools, not Python packages). ROCm runtime is a workstation system dependency, not a Python package. (021-gpu-mvp-promotion)
- Filesystem only. Reads `voter_config.yaml` (path supplied by runbook); reads host Ollama `/api/ps` at `http://localhost:11434/api/ps`; reads benchmark scratch outputs from `/tmp/021-bench/<lane>/run<N>/inv_XXX_<difficulty>/...`. Writes the four-run benchmark scratch outputs into that same `/tmp/021-bench/` tree (mirrors the corpus per-document folder contract). Writes recorded evidence into `specs/020-vendor-evidence-gate/quickstart.md` Appendix A and Appendix B, and into the new `docs/stage1-vendor-identity/runbook-gpu-mvp-demo.md`. Writes no new canonical artifact, no new `run_summary` field (FR-031, FR-032). (021-gpu-mvp-promotion)
- Python 3.12 (matches `.devcontainer/Dockerfile` base image and `pyproject.toml requires-python = ">=3.12"`; consistent with features 001–021). + existing only — `jsonschema>=4.22,<5` (Draft 2020-12), `pydantic>=2.7,<3`, Python stdlib (`unicodedata` for Q32 P* category lookup, `re` for Q10/Q32/Q35 regex, `json` for Q34 sorted-keys serialization, `decimal.Decimal` with `ROUND_HALF_EVEN` for 6-dp ratio formatting, `pathlib`, `dataclasses`). **No new pinned dependency.** No Paddle, no `paddlepaddle-dcu`, no `httpx`, no model runtime, no network. CPU-only. (022-ocr-semantic-quality-gate)
- Filesystem only. Reads `<per-doc-folder>/preprocess_output.json` and optional `<per-doc-folder>/semantic_table_truth.json`; writes `<per-doc-folder>/evaluation_document.json` (additive `semantic_table_quality` object + `document_pass_fail.semantic_table_quality_passed` field per Clarifications Q12/Q20/Q29/Q30/Q31/Q37) and `<corpus-root>/evaluation_run_summary.json` (additive top-level `semantic_table_quality_metrics` namespace per Q19/Q36). Contract set advances 1.2.0 → 1.3.0 (Q14) under `contracts/stage1_vendor_identity/v1.3.0/` with new `semantic_table_truth.schema.json` and three additive-changed schemas (`evaluation_document`, `evaluation_run_summary`, `folder`). New synthetic CPU-only US2 fixture at `tests/stage1_semantic_quality/inv_001_hard/` (hand-authored `preprocess_output.json` + sidecar, no `source.pdf` per Q25). No new canonical pipeline artifact, no runtime pipeline change (Q3 / FR-033 out of scope). (022-ocr-semantic-quality-gate)

## Recent Changes
- 023-gpu-mvp-demo-hardening (MVP + US2/US3/US4 + Phase 7 polish landed on branch `023-gpu-mvp-demo-hardening`, commit `6bb2c2e` + follow-up): canonical GPU MVP demo CLI at `python -m dartwing_ocr.gpu_demo` (also `dartwing-gpu-demo` console-script) — composes feature 003/005/008/009 via a pluggable `PipelineComposer` Protocol behind a fixed-order 8-check readiness preflight (FR-016 closed vocabulary `interpreter/venv` → `paddle-rocm-preflight` → `ollama-reachability` → `ollama-version` → `ollama-model-gpu-placement` → `ollama-context-length` → `artifact-schema-validation` → `pipeline-runtime-timeout`), a 600-second bounded SIGALRM timeout (FR-008), an always-emit stable-shape `DemoRunReport` JSON line on stdout (FR-019, 17 top-level keys with null for unknown), post-run device interrogation enforcing SC-004 (no silent CPU fallback), and a closed 0–5 exit-code table (FR-021). 28 FR + 10 SC + 21 R-023 decisions; 32 spec clarifications across 3 `/speckit-clarify` rounds + a 4th audit-walkthrough; ~2,500 lines under `src/dartwing_ocr/gpu_demo/` (25 modules incl. orchestrator, 8 readiness checks + runner, evaluator subprocess, CPU-fallback two-probe interrogation, R-023.20 truth-table `quality_status` derivation, `CheckDiagnostic` stderr-line projection); ~1,500 lines / 60 passing tests in `tests/integration/gpu_demo/`. No new pinned dependency. No contract-set bump (FR-014; v1.3.0 unchanged). Runbook extended in place at `docs/stage1-vendor-identity/runbook-gpu-mvp-demo.md` with 4 new sections + Colleague Dry-Run Sign-Off appendix (per audit Q11). Two workstation-only follow-ups tracked as GitHub issues #53 (T034a real `_DefaultPipelineComposer` wiring) and #54 (T073 `@pytest.mark.gpu` three-run stability smoke for SC-006). Coverage matrix: `specs/023-gpu-mvp-demo-hardening/coverage-fr-task-test.md`.
- 022-ocr-semantic-quality-gate (landed via stacked PRs #44 → #46 → #47 → #49 → #50; polish PR #51 open): the foundation PR (#44, merged) landed the v1.3.0 contract-set bump under `contracts/stage1_vendor_identity/v1.3.0/` with new `semantic_table_truth.schema.json` (Q-SEC-2/B safety pattern `^[A-Za-z0-9_-]{1,64}$` + `maxLength: 64` on both `document_id` and `row_id`) and the shared helpers — `src/dartwing_ocr/validator/corpus_pattern.py` (Q23 canonical-pattern allowlist `^inv_\d{3}_(easy|medium|hard)$`), `src/dartwing_ocr/evaluator/stable_json.py` (Q34 byte-identical serializer), and `SemanticGateInvariantError` (Q42/MI-19) added to the existing evaluator exception hierarchy. PR #46 (US1, merged) added the `validate semantic-truth` validator surface. PR #47 (US3, merged) wired the report surface: `semantic_table_quality` object on `evaluation_document.json` + top-level `semantic_table_quality_metrics` namespace + `semantic_document_statuses` array on `evaluation_run_summary.json`, with the gate evaluating 4 predicate-based checks in fixed Q17 order (no short-circuit) and aggregating verdicts by any-fail rule into the closed snake_case status enum {`passed`, `failed`, `not_applicable`, `unevaluable`} (Q26). PR #49 (US4, merged) landed the non-regression release gate: byte-identity vendor-identity baseline tests (SC-006 / MI-22), feature-019/020/021 protected-subtree guard, and FR-030 line-items negative assertion. PR #50 (US5, merged) landed calibration-folder governance: `validate corpus` now partitions scored vs calibration via the Q23 canonical-pattern allowlist (FR-027 / MI-20 / SC-009). Polish PR #51 (open) lands AMENDMENTS.md v1.3.0, schemas.md delta, PRD promotion, v1.3.0/README.md, CLAUDE.md cross-links, quickstart walkthrough validation, and the SC-010 / FR-032 coverage matrix at `specs/022-ocr-semantic-quality-gate/coverage-fr-task-test.md`. 44 spec Clarifications + 7 security-clarify questions resolved. FR-034 adds air-gapped operation as a named requirement (Q-SEC-7/B). Spec: `specs/022-ocr-semantic-quality-gate/spec.md`. Plan: `specs/022-ocr-semantic-quality-gate/plan.md`. Coverage: `specs/022-ocr-semantic-quality-gate/coverage-fr-task-test.md`.
- 020-vendor-evidence-gate (MVP slice): Deterministic vendor-identity evidence gate landed under `src/dartwing_ocr/preprocessing/evidence_gate.py` — five FR-001 signals (vendor-name-candidate, header-band token-density, OCR-detection aggregate-confidence, business-suffix-presence, tax-id-shaped-token-presence) computed over the page-1 header band of `preprocess_output.json`, mapped via the v1 explicit decision table to one of `sufficient` / `borderline` / `insufficient`. CPU-safe pure-Python module (stdlib + `re` only; no Paddle import). `RunSummary.SCHEMA_VERSION` patches 0.1.6 → 0.1.7 for four additive top-level fields (`evidence_gate_id`, `evidence_gate_state_counts`, `evidence_gate_documents`, `evidence_gate_suppressed_fallback_count`); always-emit on every profile. Skip-fallback behavior (US4), CPU-isolation guards (US5), schema-preservation regression tests (US6), and promotion quality gate (US7) follow as separate stacked PRs. See `specs/020-vendor-evidence-gate/quickstart.md` Path 1.
