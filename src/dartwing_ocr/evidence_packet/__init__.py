...
Persistence is gated by the ``dartwing_ocr`` logger effective level: the
...
from dartwing_ocr.evidence_packet import serialization as _serialization
from dartwing_ocr.evidence_packet.assembler import (
...
from dartwing_ocr.evidence_packet.version import PACKET_FILENAME
...
def assemble_from_folder(folder: Path) -> dict[str, Any]:
    ...
    the ``dartwing_ocr`` logger is at ``DEBUG`` or lower — writes
