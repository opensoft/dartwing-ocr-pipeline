...
from dartwing_ocr import __version__ as _package_version
...
def _ppstructurev3_factory(lane: str) -> AdapterFactory:
    ...
    ``dartwing_ocr.preprocessing.pipeline.Invocation``: ``"cpu"`` for the
    ...
    def factory(_plan: "ResolvedRunPlan") -> StageCallable:
        def adapter(
            ...
            from dartwing_ocr.pipeline.timing import current_stage_timing
...
def _ppstructurev3_cpu_factory(_plan: "ResolvedRunPlan") -> StageCallable:
    ...
    ``dartwing_ocr.preprocessing.pipeline.run``.
...
def _ollama_extract_factory(lane: str) -> Callable[["ResolvedRunPlan"], StageCallable]:
    ...
    ``dartwing_ocr.extract.pipeline.run``.

    Loads the default voter config from the on-disk YAML in
    ``src/dartwing_ocr/extract/voters/configs/gemma-edge.yaml`` and
    ...
    def factory(plan: "ResolvedRunPlan") -> StageCallable:
        ...
        from dartwing_ocr.extract.voters.ollama import OllamaVoter
...
def _routing_rules_cpu_factory(plan: "ResolvedRunPlan") -> StageCallable:
    ...
    ``dartwing_ocr.router.pipeline.run``. Pure-deterministic, no init cost.
    """
    from dartwing_ocr.router.pipeline import run as router_run
    from dartwing_ocr.router.version import (
...
def _final_payload_assembler_cpu_factory(_plan: "ResolvedRunPlan") -> StageCallable:
    ...
    ``dartwing_ocr.assembler.pipeline.run``.
    """
    import json
    from dartwing_ocr.assembler.pipeline import (
