...
from dartwing_ocr.preprocessing.rasterize import (
    PageRaster,
    PageRasterFailure,
    rasterize_pdf,
)
...
_FIXTURE_ENV_VAR: str = "DARTWING_WARMUP_FIXTURE_PATH"
...
def _resolve_default_fixture() -> Path:
    ...
    1. ``$DARTWING_WARMUP_FIXTURE_PATH`` env var (operator override —
...
def _load_fixture(fixture_path: Path) -> tuple[np.ndarray, str]:
    ...
    try:
        if not fixture_path.is_file():
            ...
            # `DARTWING_WARMUP_FIXTURE_PATH` escape hatch so they can
