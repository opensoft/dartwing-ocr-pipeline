...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "dartwing_ocr.pipeline.profiles."
        ),
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "in those cases). Can also be set via the DARTWING_GPU_WARMUP=1 "
    ...
    # --gpu-warmup. Also accepted via the DARTWING_MODULE_SET and
    # DARTWING_DET_REC_VARIANT environment variables (CLI flag wins
    # when both are set).
    p.add_argument(
        "--module-set",
        type=str,
    ...
    # Also accepted via the DARTWING_RASTER_PROFILE env var (CLI flag
    # wins when both set).
    p.add_argument(
        "--raster-profile",
        type=str,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "be set via DARTWING_REGION_STRATEGY; the CLI flag wins."
        ),
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Can also be set via DARTWING_PREPROCESS_STRATEGY; the "
            "CLI flag wins."
        ),
    )
    return p
...
def main(argv: list[str] | None = None) -> int:  # NOSONAR S3776 — CLI entry point — branches over all preprocessing flags / preset axes; splitting fragments the contract.
    ...
    if invocation.warmup and is_gpu_lane(preprocess_lane):
        from dartwing_ocr.preprocessing import warmup as _warmup_mod
