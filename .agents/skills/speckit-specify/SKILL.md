...
2. **Feature checkout creation** (optional, via hook):

   If a `before_specify` hook ran successfully in the Pre-Execution Checks above, it will have created/switched to a git feature checkout and output JSON containing `BRANCH_NAME` and `FEATURE_NUM`. In worktree mode it also returns `WORKTREE_PATH` and `BASE_BRANCH`.
...
3. **Create the spec feature directory**:
   ...
   **Create the directory and spec file**:
   - Resolve `SPECIFY_FEATURE_DIRECTORY` relative to `TARGET_REPO_ROOT` if it is not already absolute
   - `mkdir -p SPECIFY_FEATURE_DIRECTORY`
   - Copy `TARGET_REPO_ROOT/.specify/templates/spec-template.md` to `SPECIFY_FEATURE_DIRECTORY/spec.md` as the starting point
   ...
   **IMPORTANT**:
   ...
   - When `WORKTREE_PATH` is returned, all spec files and `.specify/feature.json` state are written in that worktree, not in the original checkout
...
9. **Check for extension hooks**: After reporting completion, check if `TARGET_REPO_ROOT/.specify/extensions.yml` exists.
