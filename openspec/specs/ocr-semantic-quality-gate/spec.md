# ocr-semantic-quality-gate Specification

## Purpose

Defines an optional `semantic_table_truth.json` sidecar and a deterministic semantic OCR/table-quality gate that detects confidently-wrong invoice body content (malformed currency shape, missing required row content, row-text coverage gaps, row alignment failures) without treating OCR confidence as proof of correctness. The gate runs CPU-only and Paddle-free, is harness/evaluator-scoped (not part of the stage 1 runtime pipeline), and consumes the v1.3.0 contract-set additions landed under Speckit feature 022 (`specs/022-ocr-semantic-quality-gate/`).
## Requirements
