...
- [ ] T059 ⏸ DEFERRED (corpus absent) Run corpus-calibration pass for quality thresholds against `tests/stage1_vendor_identity/inv_*/` and update `src/ledgerlinc_ocr/preprocessing/quality.py` constants if the provisional values in research.md Decision 6 misfire. Record final thresholds inline and bump `{semver}` in `version.py`.
