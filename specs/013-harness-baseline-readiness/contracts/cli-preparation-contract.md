...
```bash
python -m dartwing_ocr.evaluator document <document-folder> \
  --run-pipeline \
  --pipeline-overwrite
```
...
- The evaluator invokes `python -m dartwing_ocr.pipeline run --document-folder <document-folder>` as a subprocess.
...
```bash
python -m dartwing_ocr.evaluator corpus <corpus-root> \
  --run-pipeline \
  --pipeline-overwrite
```
...
- The evaluator writes a temporary documents file and invokes `python -m dartwing_ocr.pipeline run --documents-file <file>` as a subprocess.
