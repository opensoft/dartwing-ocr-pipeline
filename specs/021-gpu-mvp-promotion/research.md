...
**Decision**: The Ollama readiness helper takes a required `--voter-config PATH` flag pointing at a YAML file consumed **only by `scripts/check-ollama-gpu-readiness.sh`**. The runbook pins a single canonical path for the MVP demo: `configs/voter/ollama-gpu.yaml` (a NEW config file under a new `configs/voter/` directory). This config is a thin YAML wrapper that names the GPU-targeted extraction model. The pipeline demo command does NOT pass this file as a CLI argument; it selects the extraction model via `--extract-profile ollama@gpu`, which `src/dartwing_ocr/pipeline/stages.py::_ollama_extract_factory` resolves to `src/dartwing_ocr/extract/voters/configs/gemma-edge.yaml` and reads `ollama.model_tag` from. The bridge between the readiness config and the actual extractor binding is a CPU-safe contract test, `tests/contract_tests/test_voter_config_model_alignment.py`, which asserts `configs/voter/ollama-gpu.yaml::model_name == src/dartwing_ocr/extract/voters/configs/gemma-edge.yaml::ollama.model_tag`. At landing the canonical value is `model_name: "gemma4:e4b"` on both sides.

**Rationale**: (a) Existing voter-config YAMLs live in `tests/fixtures/extract/*/voter_config.yaml` — fixture-only paths unsuitable for a demo readiness check. (b) A canonical `configs/voter/ollama-gpu.yaml` gives the readiness helper a stable target and lets it resolve the model name without operator guesswork. (c) A new `configs/voter/` directory is a *configuration* surface, not pipeline code — it does not trip FR-032 (no new product behavior in `dartwing_ocr`). (d) The shell helper reads the YAML's `model_name` key (per feature 005 voter-config schema, `model_name: string (non-empty)`). (e) The two configs (readiness vs. extractor) live in different files because they serve different surfaces — readiness is operator-facing and runs before any pipeline invocation; the extractor config is package-internal and resolved at adapter-construction time. The contract test gives the two surfaces a single source of truth without coupling the CLIs.
