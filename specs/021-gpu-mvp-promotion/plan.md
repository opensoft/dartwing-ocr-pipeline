...
- **§I Boundary**: Phase 1 confirmed the Ollama readiness check is `scripts/check-ollama-gpu-readiness.sh` (POSIX shell, outside `dartwing_ocr`). The new `configs/voter/ollama-gpu.yaml` is a configuration file, not pipeline code. The new `tests/contract_tests/test_promotion_decision_sync.py` and `tests/contract_tests/test_ollama_readiness_helper_contract.py` are harness-layer tests, not pipeline modules. The four converted GPU tests under `tests/pipeline_tests/` stay in the harness layer. Pipeline / harness / model-runtime boundary intact. **PASS**.
...
├── research.md          # Phase 0 output — R-021.1 through R-021.13 decisions
