...
- **During the four-run sequence, the workstation has exclusive GPU access** — no other process is consuming `rocm-smi`-reported GPU memory beyond the running pipeline. Concurrent GPU consumers invalidate the cache-warmth invariant the four-run discipline relies on.
