...
- The pipeline is interrupted (SIGINT) partway through. → **Deferred beyond stage 1.** Default Python signal handling applies; a structured failure record is not guaranteed on forced interrupt. Whatever artifacts were already written remain on disk as a side effect of Python's normal exit path, but this is not a contract commitment at stage 1.
