...
- [ ] T040 [US1] Implement live `("routing", "rules", "cpu")` adapter binding to `ledgerlinc_ocr.router.pipeline.route` with timing phase keys `compute`/`write`, in `src/ledgerlinc_ocr/pipeline/stages.py` (replaces the `DeferredImplementationError` placeholder added in T009) (depends on T009)
- [ ] T041 [US1] Implement live `("final_payload", "assembler", "cpu")` adapter binding to `ledgerlinc_ocr.assembler.pipeline.assemble` with timing phase keys `compute`/`write`, in `src/ledgerlinc_ocr/pipeline/stages.py` (replaces the `DeferredImplementationError` placeholder added in T009) (depends on T009)
...
- [ ] T046 [US3] Implement live `("extract", "ollama", "cpu")` adapter binding to the same extract entry point as US1 but with `OllamaLaneEndpoints.cpu_url`, in `src/ledgerlinc_ocr/pipeline/stages.py` (replaces the `DeferredImplementationError` placeholder added in T009) (Spec FR-015/FR-016; Research R-012; depends on T039)
- [ ] T047 [US3] Wire profile-validation acceptance for `("preprocess", "edge-ocr", "jetson")` and `("extract", "ollama", "jetson")` in the registry: argument validation accepts these values, but adapter dispatch raises `DeferredImplementationError` with stage `prerequisite_validation`, exit code `10` (`USAGE_ERROR` per Research R-013), and a message naming FR-034 step 4 as the implementation slice; the placeholder added in T009 already raises `DeferredImplementationError` for these triples - this task tightens the message text and exit code (Spec FR-035; Research R-013/R-014; depends on T009)
