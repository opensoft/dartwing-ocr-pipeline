...
- [ ] T015 [US1] ⏸ DEFERRED per FR-025 — `@gpu` Workstation benchmark numbers — run T014 manually + record the actual `phase_timings.rasterization` and `per_page_inference` numbers for `(legacy, full-page)` and `(reduced-v1, full-page)` cells into `specs/018-dpi-region-first-preprocess/quickstart.md` Appendix A.1 / A.2. If GPU access is unavailable at landing, defer per FR-025 and capture the deferral in T038.
