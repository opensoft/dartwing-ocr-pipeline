...
- [ ] T001 Verify the `gpu` pytest marker is registered at `tests/conftest.py` (carried over from features 014–017 infrastructure). FR-023 requires this marker to exist; no new marker is added by this feature.
...
- [ ] T042 [P] Run the existing `tests/contract_tests/` suite (the v1.2.0 contract tests must continue to pass unchanged per FR-020). Confirm the existing feature-014/015/016/017 test suites also pass unchanged after this feature's changes (FR-022 carry-forward).
