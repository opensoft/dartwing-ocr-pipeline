...
- [x] CHK003 Is the `LEDGERLINC_GPU_WARMUP` env-var truthiness whitelist (`{"1", "true", "yes"}`) documented as the canonical contract — including what happens to ambiguous values like `2` or `on` — in a place a CLI-using operator will find? [Completeness, contracts/cli-contract.md §1, data-model.md §"Activation-surface state machine"]
...
- [x] CHK033 Is the case "operator passes ambiguous `LEDGERLINC_GPU_WARMUP` value" (e.g., `2`, `on`, `enabled`, empty) specified as silently-treated-as-unset rather than as an error, in a place CI scripts can rely on? [Edge Case, contracts/cli-contract.md §1, data-model.md §"Activation-surface state machine"]
