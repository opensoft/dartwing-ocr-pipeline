...
**Project Type**: single Python package (`ledgerlinc-ocr`) with pluggable preprocess profiles. No new top-level subpackage; one new module `preprocessing/warmup.py` plus surgical edits to four existing modules.
...
├── contracts/                      # Phase 1
│   ├── cli-contract.md             # `--gpu-warmup` flag + LEDGERLINC_GPU_WARMUP env var; activation orthogonality with --preprocess-profile
...
│   └── cli.py                          # CHANGED (small): wires the new `--gpu-warmup` flag; reads `LEDGERLINC_GPU_WARMUP=1` as a fallback; emits the FR-010 stderr warn-and-proceed line on CPU/stub combinations.
├── pipeline/
