...
- Pinning the cache dirs to `$HOME/.cache/miopen` and `$HOME/.cache/comgr` is what MIOpen would use anyway by default on Linux; explicitly setting the path means MIOpen does not need to compute it from incomplete HSA/runtime hints, which is what produces the workspace-warning chatter.
