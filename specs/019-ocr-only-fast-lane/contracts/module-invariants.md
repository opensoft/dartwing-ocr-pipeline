...
Only the four values in `R-019.2` (`"ppstructurev3"`, `"ocr-only-v1"`, `"cpu-default"`, `"stub-default"`) are valid for `preprocess_strategy_id`. Any other value passed via `--preprocess-strategy` or `LEDGERLINC_PREPROCESS_STRATEGY` MUST raise `UnknownPresetError(preset_axis="preprocess_strategy", ...)` → exit code 16. The registry `PREPROCESS_STRATEGIES` is frozen at module-load time; runtime mutation is forbidden.
