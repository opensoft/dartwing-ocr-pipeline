...
- All new code files live under `src/dartwing_ocr/preprocessing/` or `src/dartwing_ocr/pipeline/`; all new test files under `tests/unit/preprocessing/` or `tests/pipeline_tests/` per Plan § Source Code
...
- [x] T001 Verify the editable install is current — run `.venv/bin/pip install -e ".[dev]"` from the worktree root and confirm `python -c "import dartwing_ocr"` succeeds without errors
...
- [x] T004 Add identifier constants to `src/dartwing_ocr/preprocessing/identifiers.py` — `EVIDENCE_GATE_ID_V1 = "v1"` and `EVIDENCE_GATE_ID_DEFAULT = EVIDENCE_GATE_ID_V1`; preserve all existing feature 017/018/019 strings unchanged (Plan § Source Code; R-020.2; data-model.md Appendix A)
...
- [x] T009 [US1] Create `src/dartwing_ocr/preprocessing/evidence_gate.py` — module scaffolding: `Y_THRESHOLD_FRACTION = 0.25`, `DENSITY_THRESHOLD = 8`, `CONFIDENCE_THRESHOLD = 0.70` (all `Final[...]`); `VENDOR_NAME_STOP_WORDS: Final[frozenset[str]]` per data-model.md §8; compile `BUSINESS_SUFFIX_RE`, `TAX_ID_EIN_RE`, `TAX_ID_VAT_RE` per R-020.4 / data-model.md §6 (CPU-safe — only `re` and stdlib at module load; no Paddle, no `paddleocr`, no `paddlepaddle` imports — MI-4 / MI-5)
- [x] T010 [US1] In `src/dartwing_ocr/preprocessing/evidence_gate.py`, add the frozen dataclass `FiveSignalSet` per data-model.md §2 with the five typed fields and validation rules (no `NaN`, no `Infinity`, `ocr_detection_confidence_mean ∈ [0.0, 1.0]`)
- [x] T011 [US1] In `src/dartwing_ocr/preprocessing/evidence_gate.py`, implement `_extract_header_band_tokens(preprocess_output: dict, *, y_threshold_fraction: float = 0.25)` — apply NFKC normalization to each block/box's recognized text, tokenize on whitespace, filter by `bbox_top_y / page_height < y_threshold_fraction` on `pages[0]` only (R-020.3 / R-020.5)
- [x] T012 [US1] In `src/dartwing_ocr/preprocessing/evidence_gate.py`, implement the five signal computations as private helpers operating on the band-token list and the band's detection-box confidences; ensure empty-band returns the negative-level tuple per R-020.3 / data-model.md §2
- [x] T013 [US1] In `src/dartwing_ocr/preprocessing/evidence_gate.py`, expose `evaluate_evidence_gate(preprocess_output: dict, *, gate_id: str = "v1") -> EvidenceGateResult` returning the populated `FiveSignalSet` (decision wiring lands in US2); the function MUST accept only the `preprocess_output` dict (MI-2 / MI-3)
...
- [x] T025 [US3] In `src/dartwing_ocr/pipeline/timing.py`, bump `SCHEMA_VERSION = "0.1.6"` → `SCHEMA_VERSION = "0.1.7"` (R-020.9)
- [x] T026 [US3] In `src/dartwing_ocr/pipeline/timing.py`, extend `RunSummary` (dataclass) with four new fields per data-model.md §5: `evidence_gate_id: str = EVIDENCE_GATE_ID_DEFAULT`, `evidence_gate_state_counts: dict[str, int]` (default factory returns `{"sufficient": 0, "borderline": 0, "insufficient": 0}`), `evidence_gate_documents: list[dict]` (default factory returns `[]`), `evidence_gate_suppressed_fallback_count: int = 0`; preserve every existing field name/type/default unchanged (FR-011 / FR-022 / MI-19)
- [x] T027 [US3] In `src/dartwing_ocr/pipeline/timing.py`, update `RunSummary.to_dict()` to emit the four new keys in deterministic order AFTER feature 019's two fields and BEFORE any future additive field (R-020.10 / run-summary-schema.md)
- [x] T028 [US3] In `src/dartwing_ocr/pipeline/corpus_run.py`, after each document's preprocessing completes, call `evaluate_evidence_gate(<final preprocess_output dict>)` and: (a) increment `evidence_gate_state_counts[result.decision]`; (b) append an `EvidenceGateDocumentRecord`-shaped dict (`document_id` + `decision` + nested `signals` object per data-model.md §4) to `evidence_gate_documents`; iteration order follows the existing per-document iteration (typically alphabetical by `document_id` — R-020.11)
- [x] T029 [US3] In `src/dartwing_ocr/pipeline/runner.py`, mirror T028's wiring for the single-document path — evaluate the gate on the document's final `preprocess_output.json` and populate the four new `RunSummary` fields; `document_id` derived from the input folder's basename (R-020.11)
...
- [x] T031 [P] [US4] Author `tests/unit/preprocessing/test_evidence_gate_optin_unit.py` — cover `resolve_evidence_gate_skip_fallback` precedence: CLI True wins over any env value; CLI False (absent) + truthy env (`"1"`, `"true"`, `"yes"`, `"on"`, case-insensitive) → True; CLI False + falsy env (`"0"`, `"false"`, `"no"`, `"off"`, `""`, unset) → False; unrecognized env value rejects via the same error path as existing `_PRESET_ENV_VAR` helpers (R-020.1); assert `evidence_gate_skip_fallback_warn_message(active_profile)` returns a string containing the literal substring `--evidence-gate-skip-fallback ignored:` (MI-22)
- [x] T032 [P] [US4] Author `tests/unit/preprocessing/test_evidence_gate_suppress_predicate.py` — exhaustive 16-row truth-table coverage of `should_suppress_fallback(preprocess_strategy_id, fr_005_trigger_would_fire, opt_in_active, candidate_gate_decision)`; assert it returns `True` iff ALL four conjuncts hold; assert `borderline` and `insufficient` candidate decisions NEVER trigger suppression regardless of the other three inputs (MI-13 / MI-14 / FR-009 / SC-011)
- [ ] T033 [P] [US4] [DEFERRED per R-020.15] Author `tests/pipeline_tests/test_evidence_gate_skip_fallback.py` — mark with `@pytest.mark.gpu` (FR-024 / R-020.15); GPU end-to-end run on a `sufficient`-eligible fixture under `--preprocess-strategy=ocr-only-v1 --evidence-gate-skip-fallback`; assert `evidence_gate_suppressed_fallback_count == 1`, `ocr_only_fallback_count == 0` for the document, the document's `evidence_gate_documents` entry records `decision: "sufficient"`, and the final `preprocess_output.json` is the OCR-only candidate (R-020.7 / R-020.8). **Current state:** placeholder file landed with `@pytest.mark.skip`; GPU assertions to be implemented in the follow-up GPU run.
- [~] T033a [P] [US4] [CPU done; GPU DEFERRED per R-020.15] Author `tests/pipeline_tests/test_evidence_gate_all_suppressed_lazy_construction.py` — covers the FR-007 lazy-construction clause. **`test_lazy_no_warmup` (GPU @pytest.mark.gpu, DEFERRED per R-020.15)**: placeholder landed; GPU assertions (`_ENGINE` is None, `phase_timings.warmup == 0`, etc.) to be implemented in the follow-up GPU run. **`test_all_sufficient_corpus_disposes_as_suppress` (CPU-safe — merge-gating, LANDED)**: on a multi-doc corpus where every candidate evaluates to `sufficient` AND opt-in active AND trigger fires, every per-document `decide_ocr_only_fallback_disposition` call returns `"suppress"`. (Originally specified with a recording-stub-fails-if-called assertion against the PPStructureV3 factory; replaced with the disposition-level assertion because the stub was never wired into production code — the real factory-not-called assertion lives in the GPU variants.) **`test_forced_construction_with_warmup` (GPU @pytest.mark.gpu, DEFERRED)**: placeholder landed; GPU assertions to be implemented in the follow-up GPU run.
- [ ] T034 [P] [US4] [DEFERRED per R-020.15] Author `tests/pipeline_tests/test_evidence_gate_skip_fallback_borderline.py` — mark with `@pytest.mark.gpu`; same setup as T033 but on a `borderline`-eligible fixture; assert `evidence_gate_suppressed_fallback_count == 0`, `ocr_only_fallback_count == 1`, the document's `evidence_gate_documents` entry records the gate decision over the POST-fallback PPStructureV3 output (MI-11 / R-020.7). GPU-only; CPU-safe coverage of the same MI-11 invariant lives in T034a (split out for I2 / clarity). **Current state:** placeholder file landed with `@pytest.mark.skip`; GPU assertions to be implemented in the follow-up GPU run.
- [x] T034a [P] [US4] Author `tests/pipeline_tests/test_evidence_gate_skip_fallback_borderline_cpu.py` — CPU-safe variant of T034 using an injected fake `preprocess_strategy` whose fallback path is observable; assert MI-11 (gate evaluated TWICE per document — once on the OCR-only candidate for the suppression decision, once on the post-fallback output for the recorded decision) without requiring GPU; assert the recorded `evidence_gate_documents` entry reflects the post-fallback evaluation, not the candidate evaluation; merge-gates US4 even if T034 is deferred per R-020.15
- [x] T035 [P] [US4] Author `tests/pipeline_tests/test_evidence_gate_recorded_over_final.py` — CPU-safe via injection; assert that for a fixture where the candidate evaluation differs from the final evaluation, the RECORDED decision in `evidence_gate_documents` matches the gate evaluation over the FINAL `preprocess_output.json` (MI-10 / R-020.7)

### Implementation for User Story 4

- [x] T036 [US4] Create `src/dartwing_ocr/preprocessing/evidence_gate_optin.py` — CPU-safe module mirroring `preprocess_strategy_optin.py` exactly; export `EVIDENCE_GATE_SKIP_FALLBACK_ENV_VAR = "DARTWING_EVIDENCE_GATE_SKIP_FALLBACK"`, `resolve_evidence_gate_skip_fallback(cli_value: bool | None, env: Mapping[str, str] | None = None) -> bool`, and `evidence_gate_skip_fallback_warn_message(active_profile: str) -> str` returning a string containing the literal `--evidence-gate-skip-fallback ignored: active profile is not ppstructurev3@gpu` grep-able marker (R-020.1 / R-020.12 / MI-22)
- [x] T037 [US4] In `src/dartwing_ocr/preprocessing/evidence_gate.py`, implement `should_suppress_fallback(*, preprocess_strategy_id, fr_005_trigger_would_fire, opt_in_active, candidate_gate_decision) -> bool` per R-020.8 / MI-13; pure function, no globals, four-conjunct predicate. The predicate is colocated with the gate body (not in `evidence_gate_optin.py`) because it consumes `candidate_gate_decision`, which is a gate output — the opt-in module owns CLI/env resolution and warn-message strings only
- [x] T038 [US4] In `src/dartwing_ocr/preprocessing/cli.py`, add the `--evidence-gate-skip-fallback` boolean flag (default `False`) and wire it through the existing CLI parsing path; help-text excerpt per `contracts/cli-contract.md` § Help text; place adjacent to feature 019's `--preprocess-strategy`
- [x] T039 [US4] In `src/dartwing_ocr/preprocessing/cli.py`, call `resolve_evidence_gate_skip_fallback(cli_value, env)` to compute the active opt-in; when resolved `True` AND active profile is NOT `ppstructurev3@gpu`, emit exactly one stderr line via `evidence_gate_skip_fallback_warn_message(active_profile)` and continue with the run unchanged (warn-and-proceed, MI-22 / MI-23 / R-020.12); same exit code as the no-flag run
- [x] T040 [US4] In `src/dartwing_ocr/preprocessing/pipeline.py` (`run_preprocess`), integrate the gate per the R-020.7 evaluation order: (1) preprocessing produces a candidate `preprocess_output.json`; (2) if `preprocess_strategy_id == "ocr-only-v1"` AND the feature 019 FR-005 trigger fires AND the opt-in is active, evaluate `evaluate_evidence_gate(<candidate>)`; (3) if the candidate decision is `sufficient`, suppress fallback (keep OCR-only as final) AND increment `evidence_gate_suppressed_fallback_count` on the run-level `RunSummary`; (4) otherwise run feature 019's existing PPStructureV3 fallback unchanged and re-evaluate the gate on the post-fallback output for the recorded decision; the RECORDED decision is always the gate evaluation over the FINAL `preprocess_output.json` (R-020.7 / MI-10 / MI-11 / MI-12). **I3 fix — injection seam for T034a**: while wiring the fallback branch, factor the "(strategy_id, fr_005_trigger_would_fire) → fallback_or_keep" decision so it can be exercised in CPU-safe tests without invoking PaddleOCR. Mirror the prior-feature pattern used by `preprocess_strategy_optin` (feature 019) and `region_strategy_optin` (feature 018): expose either (a) a module-level helper that takes preprocessing-pass state as keyword args and returns the disposition (suppress / fallback / keep), or (b) a `preprocess_strategy_factory` parameter on `run_preprocess` accepting a stub. T034a uses whichever seam this task lands; if neither pattern fits, document the chosen seam shape in `contracts/module-invariants.md` as a new MI under "Evaluation-order invariants"
- [x] T041 [US4] In `src/dartwing_ocr/pipeline/cli.py`, add the same `--evidence-gate-skip-fallback` flag + env-var fallback wiring as T038 + T039 (warm-corpus path); preserve the orthogonal-composition precedent established by features 014–019; help-text identical to the preprocessing CLI
...
**Independent Test**: On a host without Paddle GPU, run the default test suite (`.venv/bin/pytest -m "not gpu"`) and confirm exit 0 — including all signal-set and gate-decision unit tests. Run `python -m dartwing_ocr.preprocessing --evidence-gate-skip-fallback --preprocess-profile ppstructurev3@cpu ... 2>&1 | grep -F -- "--evidence-gate-skip-fallback ignored:"` and confirm exit 0 with the grep match.

### Tests for User Story 5 ⚠️

- [x] T043 [P] [US5] Author `tests/unit/preprocessing/test_cpu_warn_and_proceed_evidence_gate.py` — CPU-safe; invokes the CLI with `--evidence-gate-skip-fallback` on `ppstructurev3@cpu` and stub-adapter profiles; asserts exactly ONE stderr line matches the grep-able marker (MI-22); asserts the four new `run_summary` fields are still emitted with default values (gate runs on CPU per FR-014); asserts the exit code equals the no-flag run on the same fixture (MI-23); covers the env-var fallback path with `DARTWING_EVIDENCE_GATE_SKIP_FALLBACK=1`
...
- [x] T045 [P] [US5] Author `tests/unit/preprocessing/test_evidence_gate_cpu_isolation.py` — assert no CPU code path imports any GPU-only module from feature 019's preprocess-strategy machinery (FR-014); use the existing test pattern from feature 016/017/018/019's CPU-isolation tests

### Implementation for User Story 5

- [x] T046 [US5] Verify (no code change expected — pure audit task) that `src/dartwing_ocr/preprocessing/evidence_gate.py` and `src/dartwing_ocr/preprocessing/evidence_gate_optin.py` import only `re`, `typing`, `dataclasses`, `collections.abc`, and other stdlib symbols at module load (no `paddleocr`, no `paddlepaddle`, no `paddle` import); if any GPU-only import slipped in via integration tasks T036–T042, refactor to guard the import (lazy / conditional) so MI-4 / MI-5 / FR-014 still hold — **verified**: `evidence_gate.py` imports only `re` / `statistics` / `unicodedata` / `dataclasses` / `typing` + `preprocessing.identifiers`; `evidence_gate_optin.py` imports only `os` + `typing`. No Paddle / paddleocr / paddlepaddle / torch / tensorflow / numpy / PIL / pypdfium2 / `preprocessing.{ocr,ocr_only,warmup,preflight,pipeline,rasterize}` import on either module — confirmed by static source scan and now machine-enforced by T045's `test_evidence_gate_cpu_isolation.py`. No refactor needed.
- [x] T047 [US5] Verify (no code change expected — pure audit task) that `src/dartwing_ocr/pipeline/timing.py`, `src/dartwing_ocr/pipeline/corpus_run.py`, and `src/dartwing_ocr/pipeline/runner.py` still import-clean on a no-Paddle host; integrate `tests/unit/preprocessing/test_evidence_gate_module_safety_unit.py` (T008) into the default CI selection to enforce this on every PR — **verified**: `timing.py` imports only `contextvars` / `json` / `sys` / `time` / `contextlib` / `dataclasses` / `typing` + `pipeline.profiles` + `preprocessing.identifiers`; `corpus_run.py` imports only stdlib + pipeline sibling modules + `preprocessing.errors` + `preprocessing.warmup_optin` (itself CPU-safe per its own MVP/T008 audit); `runner.py` imports only stdlib + pipeline siblings + `validator.{artifact,loader,report}`. No top-level `paddle` / `paddleocr` import on any of the three (`grep -E '^(import|from) ' | grep -iE 'paddle'` returns empty). The CPU regression suite (`pytest -m "not gpu" tests/contract_tests/ tests/unit/ tests/pipeline_tests/`) collects and runs these modules every PR — net-new failure delta is 0 in this US5 PR, so the import-clean property holds.
...
- [x] T048 [P] [US6] Author `tests/pipeline_tests/test_legacy_byte_identity_evidence_gate.py` — CPU variant; a run with NO `--evidence-gate-skip-fallback` flag and no truthy env var produces a `preprocess_output.json` byte-identical to a pre-feature-020 baseline on the same fixture (SC-006 / SC-007 / FR-019 / MI-20); compare against a captured baseline under `tests/fixtures/` (reuse the feature 019 baseline if available). **Result**: two tests authored — (a) round-trip determinism (MI-20: two default-profile runs on the same fixture produce byte-identical `preprocess_output.json`), (b) committed-baseline comparison against `tests/fixtures/feature_020_baseline/preprocess_output_cpu_default_inv_001_easy.json` (captured on first run on this branch tip — see test docstring for the rationale of capturing on this branch vs. attempting a pre-feature-020 reproduction). Both pass on CPU stub adapter.
- [x] T049 [P] [US6] Author `tests/pipeline_tests/test_canonical_artifacts_unchanged.py` — assert `git diff main -- contracts/stage1_vendor_identity/` shows zero changes attributable to this feature's PR (SC-010); assert `contract_set_version` reads the same value as on `main` (FR-020); run as a CPU-safe regression check. **Result**: two tests authored — (a) name-only diff against `origin/main` / `main` over `contracts/stage1_vendor_identity/` must be empty, (b) `contract_set_version` string read from working tree must equal the string read from the base ref (via `git show`). Both pass; manual `git diff main -- contracts/stage1_vendor_identity/` exits 0.
- [x] T050 [P] [US6] Author `tests/pipeline_tests/test_downstream_stages_accept_outputs.py` — end-to-end run on at least one fixture (CPU variant; GPU end-to-end deferred per R-020.15); preprocess → evidence packet → extract (stub voter) → route → assemble → evaluate; assert every emitted artifact validates against its `contracts/stage1_vendor_identity/v1.2.0/*.schema.json`; assert no canonical artifact contains any gate-related field (FR-021 / SC-007). **No-sidecar assertion (FR-021)**: snapshot the per-document folder's file list (e.g., `set(p.name for p in folder.iterdir())`) BEFORE the run; after the run, assert the only NEW files are the four canonical artifacts (`preprocess_output.json`, `edge_extraction_output.json`, `routing_decision.json`, `final_structured_payload.json`) — no `evidence_gate.json` / `gate_decision.json` / `signals.json` / any other sidecar appears. Optional debug-mode `page_*.png` are tolerated only if they pre-existed in the snapshot. **Result**: two e2e test functions authored (cold `--document-folder` path AND warm-corpus `--documents-file` path with the gate firing), sharing a `_assert_canonical_artifacts_present_valid_and_gate_clean()` helper that runs (a) absolute folder-state check — `post_snapshot == pre_snapshot ∪ RESERVED_ARTIFACT_NAMES` (catches new sidecars AND pre-existing pollution), (b) schema validation of each artifact under v1.2.0 via `validate_artifact`, (c) recursive key walk filtered by an `_is_forbidden_gate_field()` predicate — prefix-based check against `evidence_gate*` / `gate_decision*` plus an explicit `_GATE_SIGNAL_FIELDS` list for the five deliberately-non-namespaced signal names per R-020.3 (catches new gate fields without manual list maintenance), (d) module-load `_ARTIFACT_MAP` ⊆ `RESERVED_ARTIFACT_NAMES` drift guard so adding a new canonical artifact to one without the other fails loudly. All pass on CPU stub adapter.

### Implementation for User Story 6

- [x] T051 [US6] Verify (no code change expected — pure audit task) that nothing in `evidence_gate.py`, `evidence_gate_optin.py`, the CLI wiring, or the `RunSummary` changes touches `contracts/stage1_vendor_identity/v1.2.0/*.schema.json` or `contract_set.json`; if any change is detected, revert and route through `contracts/stage1_vendor_identity/AMENDMENTS.md` instead (FR-020). **Audit result**: (a) `evidence_gate_optin.py` does not exist on this MVP branch — it lands in parallel via US4 (when US4 merges, re-run this audit to extend coverage). (b) `git diff main -- contracts/stage1_vendor_identity/` exits 0 with empty output. (c) `grep -n "contracts/" src/dartwing_ocr/preprocessing/evidence_gate.py src/dartwing_ocr/preprocessing/cli.py src/dartwing_ocr/pipeline/corpus_run.py src/dartwing_ocr/pipeline/timing.py` returns only documentation-comment references to `specs/020-vendor-evidence-gate/contracts/...` and `contracts/run-summary-schema.md` (the run_summary contract is a Markdown spec, NOT a `contracts/stage1_vendor_identity/` schema). No write path to canonical contracts directory. (d) `test_canonical_artifacts_unchanged.py` (T049) locks this invariant in the test suite. Status: clean.
- [x] T052 [US6] Run the existing `tests/contract_tests/` suite against the post-feature-020 branch tip and confirm exit 0 (FR-020 / SC-009); document the result inline in this task before marking done. **Result**: `PYTHONPATH=src /workspace/projects/dartwing/ocr-pipeline/.venv/bin/python -m pytest tests/contract_tests/ -q` — 135 passed, 0 failed, 1 warning (unrelated ccache notice from paddle). Exit code 0. Schema-set v1.2.0 contracts continue to pass unmodified per FR-020 / SC-009.
- [x] T052a [P] [US6] Author `tests/pipeline_tests/test_evidence_gate_counter_invariance.py` — CPU-safe; construct a `RunSummary` with `region_strategy_fallback_count=2` and `ocr_only_fallback_count=1` (non-zero starting values); run the gate aggregation path (T028's wiring); assert both counters remain at their input values (`2` and `1` respectively) — the gate MUST NOT alter, suppress, or shadow either counter (FR-023). Symmetric assertion: set `evidence_gate_state_counts` to non-zero values; run feature 018's region-strategy fallback path; assert the gate counters remain at their input values. **Result**: three tests authored — (a) forward direction: `RunSummary.to_dict()` with non-zero gate state preserves feature 018/019 counters verbatim, (b) reverse direction: simulated post-construction writes to 018/019 counters leave `evidence_gate_*` fields snapshot-equal, (c) `evaluate_evidence_gate()` is a pure function — three interleaved gate evaluations with counter writes between them leave both counters at the arithmetically expected values. All pass on CPU.
...
- [x] T053 [P] [US7] Author `tests/pipeline_tests/test_evidence_gate_benchmark.py` — mark with `@pytest.mark.gpu` (R-020.15 deferrable); GPU run over the fixed 5-doc subset (R-020.13). **5-doc subset lookup procedure**: (1) FIRST, read `specs/017-ppstructurev3-module-reduction/quickstart.md` Appendix A and `specs/019-ocr-only-fast-lane/quickstart.md` § FR-015 benchmark table — if either has been filled at landing time with three concrete `inv_*` folder names, use that exact list to preserve cross-feature comparability per R-020.13. (2) If neither has been filled (as of 2026-05-16 both list the 3 medium/hard docs as TBD), use this candidate subset matching the R-017.11 constraint of "2 easy + 1 mid-difficulty + 2 challenging": `inv_001_easy`, `inv_002_easy`, `inv_006_medium`, `inv_011_hard`, `inv_012_hard`. (3) Record the actual subset used in `specs/020-vendor-evidence-gate/quickstart.md` Appendix A at landing time so the subset is auditable. **Four-run benchmark discipline (R-020.16)**: (a) invoke `--gpu-warmup` ONCE at the start of the benchmark session to populate MIOpen / COMGR caches and warm engine pools (feature 016 / 019 precedent); (b) run the legacy default (no opt-in) over the 5-doc subset TWICE, discard run 1's timings as additional warm-in, record run 2's `phase_timings.*` as the legacy baseline; (c) run the skip-fallback candidate (opt-in active) over the same subset TWICE, discard run 1, record run 2 as the candidate; (d) for each document, compute the latency delta `(legacy_total - candidate_total)` AND the host's run-to-run jitter band (absolute spread between the two legacy `phase_timings.total` values on that document); (e) report all FOUR runs' `phase_timings.*` values for EVERY emitted key per document (`paddle_import`, `gpu_bind_probe`, `engine_init`, `warmup`, `rasterization`, `per_page_inference`, `artifact_write`, `total` per feature 014/015/016 timing surface) in `quickstart.md` Appendix A, plus the per-doc jitter band on each key (absolute spread between the two legacy runs for that key). **Per-key change assertion (FR-015 per-key change clause, Clarifications Session 2026-05-16 Q1 Option A)**: in the test body, for each suppressed document (one with `evidence_gate_suppressed_fallback_count` incrementing on that doc), assert ONLY `phase_timings.per_page_inference` and `phase_timings.total` decrease vs. legacy run 2 on that document; assert `paddle_import`, `gpu_bind_probe`, `engine_init`, `warmup`, `rasterization`, `artifact_write` stay within the per-key jitter band (defined per key from step (d)). For unsuppressed documents (where candidate gate decision was `borderline` or `insufficient` and the fallback ran), assert all keys stay within their respective jitter bands. The benchmark also emits per-doc gate decisions, per-corpus `evidence_gate_state_counts`, and per-doc `evidence_gate_suppressed_fallback_count` increments; persists the run 2 `run_summary` JSON for use by T056 (no new artifact written to the corpus — Plan § Storage). Cold-cache runs (after `~/.cache/miopen` clearance) are explicitly OUT of FR-015 scope per R-020.16
- [x] T054 [P] [US7] Author `tests/pipeline_tests/test_quality_gate_two_metric_evidence_gate.py` — mark with `@pytest.mark.gpu`; read `evaluation_run_summary.json` from both the legacy and candidate runs over the 5-doc subset; assert `candidate aggregate field score >= legacy aggregate field score` AND `candidate per-document pass count >= legacy per-document pass count`; fail the test (NOT skip) if either metric regresses, so a failing promotion candidate is visibly rejected (FR-016 / R-020.14 / SC-008). **Landing note**: skeleton landed with `pytestmark = pytest.mark.gpu` at module level. The repo's `pyproject.toml` does NOT configure a default `-m` marker filter — a plain `pytest` invocation collects this test and skips it via the `tests/conftest.py` gpu-marker gate (which skips when preflight ≠ `ppstructurev3_init_succeeded`). Operators can additionally pass `pytest -m "not gpu"` to deselect at marker-filter time as an alternative. On a GPU host where preflight succeeds AND no `-m` excludes the test, the body raises `pytest.fail` so the deferred-implementation state surfaces loudly the moment GPU verification is attempted. Prior layered `@pytest.mark.skip` + `pytest.fail` was simplified to gpu-marker-only per review polish `f695a96`: the conftest gate already handles runtime skip; the explicit `@pytest.mark.skip` was redundant on no-GPU hosts and blocked auto-activation on GPU hosts. The two-metric assertion shape is documented in the test docstring using the actual `evaluation_run_summary.schema.json` field names (`overall_metrics.vendor_identity_pass_rate` + `sum(documents[].overall_passed)`); activation is gated on the deferred GPU run captured by T058.

### Implementation for User Story 7

- [x] T055 [US7] Confirm `MI-20` invariant holds: at landing, `resolve_evidence_gate_skip_fallback(cli_value=None, env=<empty>)` returns `False`; the opt-in default is OFF on every profile. No code change required if T036 wired the default correctly; if not, fix and re-run T031. **Landing note**: confirmed — `resolve_evidence_gate_skip_fallback(cli_value=None, env={})` returns `False` per `src/dartwing_ocr/preprocessing/evidence_gate_optin.py`; covered by `test_mi_20_default_off_invariant` in `tests/unit/preprocessing/test_evidence_gate_optin_unit.py` (CPU-safe; the broader optin test suite there already covers the precedence axes).
- [x] T056 [US7] If GPU is available at landing, run T053 + T054 and record the resulting quality-gate evidence in `specs/020-vendor-evidence-gate/research.md` Appendix B using the table format documented there (legacy aggregate_score / candidate aggregate_score / legacy pass / candidate pass per document plus TOTAL row); also record the FR-015 benchmark numbers in `specs/020-vendor-evidence-gate/quickstart.md` Appendix A using the table format documented there. If GPU is NOT available, mark Appendix B with the explicit deferral cross-reference per T058 below. **Landing note**: GPU not available on this runner (CPU-only container); deferral notes appended to `research.md` Appendix B and `quickstart.md` Appendix A cross-referencing T058 (surveillance follow-up). Appendix table structures intentionally left unchanged for the deferred GPU run to fill.
- [x] T057 [US7] If quality-gate passes AND the team chooses to promote: flip the default in `src/dartwing_ocr/preprocessing/evidence_gate_optin.py::resolve_evidence_gate_skip_fallback` (the only place the default lives); record the promotion in `research.md` Appendix B alongside the `evidence_gate_id="v1"` reference (FR-017 / FR-018 / MI-21). **Post-promotion verification (FR-018)**: after flipping the default, run the existing CLI accepting an explicit-off configuration (`DARTWING_EVIDENCE_GATE_SKIP_FALLBACK=0 python -m dartwing_ocr.preprocessing ...` and equivalent for the pipeline CLI); assert the legacy non-suppression behavior is reproduced (no suppression, `evidence_gate_suppressed_fallback_count == 0`); add a regression test under `tests/pipeline_tests/test_legacy_behavior_selectable_after_promotion.py` (or extend T043) that asserts the explicit-off path remains selectable. **If quality-gate fails OR the team makes no promotion decision, this task closes as "default unchanged; legacy behavior preserved" — NOT as a failure.** **Landing note**: closed as "default unchanged; legacy behavior preserved" per the spec's no-promotion path — no GPU evidence available at landing to evaluate (T056 deferred). The default in `evidence_gate_optin.py::resolve_evidence_gate_skip_fallback` remains OFF and is exercised by `test_mi_20_default_off_invariant` (T055). Promotion is gated on a passing `test_quality_gate_two_metric_evidence_gate.py` verdict in a follow-up GPU run.
...
Task: "T036 src/dartwing_ocr/preprocessing/evidence_gate_optin.py (new module)"
Task: "T037 should_suppress_fallback predicate in preprocessing/evidence_gate.py"

# CLI wiring across two CLIs in parallel:
Task: "T038 + T039 src/dartwing_ocr/preprocessing/cli.py (flag + warn-and-proceed)"
Task: "T041 src/dartwing_ocr/pipeline/cli.py (flag + warn-and-proceed)"

# Then integrate (single file — serial):
Task: "T040 src/dartwing_ocr/preprocessing/pipeline.py (R-020.7 evaluation order + R-020.8 suppression)"
Task: "T042 src/dartwing_ocr/pipeline/corpus_run.py (per-doc counter aggregation)"
```
