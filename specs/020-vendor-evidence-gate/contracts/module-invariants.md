...
| MI-8 | `Y_THRESHOLD_FRACTION`, `DENSITY_THRESHOLD`, `CONFIDENCE_THRESHOLD`, and the regex constants MUST be declared `Final[...]` and have their landing values pinned. | `Final[...]` is a type-checker-level hint and is not enforced at runtime by Python. The repository does not currently run mypy/pyright in CI, so the static guarantee is reviewer-enforced rather than tooling-enforced (Phase 6 review correction). The pinned landing values ARE runtime-asserted by `test_evidence_gate_registry_unit.py::test_threshold_constants_pinned` and `test_module_constants_are_deterministic`. |
...
| # | Invariant | Enforcement |
|---|---|---|
| MI-10 | The decision recorded in `evidence_gate_documents` for any document MUST be the gate evaluation over that document's FINAL `preprocess_output.json` (the file written to disk and accepted by downstream stages). | `test_evidence_gate_pipeline_integration.py` exercises the real on-disk path: writes a `preprocess_output.json` fixture, invokes `evaluate_and_record` (the production helper used by both `corpus_run.py` and `cli.py`), and asserts the per-doc record's decision matches the gate's reapplication to that file. |
...
| # | Invariant | Enforcement |
|---|---|---|
| MI-16 | Every run of `python -m dartwing_ocr.preprocessing` or `python -m dartwing_ocr.pipeline` MUST emit a `kind: "run_summary"` stdout line that includes all four new top-level fields (`evidence_gate_id`, `evidence_gate_state_counts`, `evidence_gate_documents`, `evidence_gate_suppressed_fallback_count`). | `test_run_summary_schema_0_1_7.py::test_four_new_fields_present_with_defaults` constructs a `RunSummary` directly and asserts all four keys land on the emitted dict via `to_dict()`. Real-CLI stdout invocations (`jq -e`-style assertions over `python -m ...` output) are validated by the smoke-test items in `quickstart.md`'s Path 1 / Path 2 / Path 7 walkthroughs (Phase 6 correction — the previous text overstated this as an in-PR test). |
...
| MI-18 | `evidence_gate_state_counts[s]` MUST equal the count of `evidence_gate_documents[i].decision == s` for each `s` in the closed vocabulary. | `test_evidence_gate_runsummary_aggregation.py` asserts the aggregate equals the per-document count for each of the three states at the synthetic-RunSummary level; `test_evidence_gate_pipeline_integration.py::test_evaluate_and_record_aggregates_across_multiple_docs` asserts the same invariant when the production helper runs over multiple on-disk fixtures; `test_evidence_gate_callsite_regression.py` asserts the production call sites in `corpus_run.py` and `cli.py` still invoke `evaluate_and_record` (the only place the accumulators are updated). |
...
| MI-26 | `decide_for_gate` and `evaluate_evidence_gate` MUST return a decision in `{"sufficient", "borderline", "insufficient"}` and nothing else (no fourth state, no `None`, no string outside the closed vocabulary). | The return type annotation is `Literal["sufficient", "borderline", "insufficient"]` (type-checker hint; not enforced at runtime — no mypy/pyright in CI per Phase 6 review). Runtime tests cover the boundary at all three thresholds and assert the return is in the closed vocabulary via a parameterized table over the 32-row truth table (`test_evidence_gate_decision_unit.py`, `test_evidence_gate_registry_unit.py::test_decide_returns_closed_vocabulary`). |
