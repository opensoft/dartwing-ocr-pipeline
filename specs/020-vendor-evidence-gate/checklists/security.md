...
- [x] CHK013 Is each pattern bounded in repetition (no unbounded `.*` or `.+` outside word boundaries) — specifically `BUSINESS_SUFFIX_RE` is a fixed alternation with `(?![\w-])` end-lookahead, `TAX_ID_EIN_RE` is `\d{2}-\d{7}` (fully bounded), `TAX_ID_VAT_RE` is `[A-Z]{2}(?=[A-Z0-9]{2,12}\b)[A-Z0-9]*\d[A-Z0-9]*` (bounded to 14 chars via the lookahead AND requires ≥1 digit per B2 post-review)? [Clarity, R-020.4]
