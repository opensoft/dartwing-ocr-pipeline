...
**T056 deferral note (US7 landing, CPU-only runner)**: GPU benchmark + quality-gate verification deferred per R-020.15. Cross-reference: `tasks.md` T058 — surveillance follow-up captures the deferred GPU runs (`test_evidence_gate_benchmark.py` produces the numbers above; `test_quality_gate_two_metric_evidence_gate.py` produces the candidate-vs-legacy verdict). Both test skeletons are present in `tests/pipeline_tests/` with module-level `pytestmark = pytest.mark.gpu`. Two independent gates keep them inert on non-GPU hosts: (1) pytest's `-m "not gpu"` expression (default CI invocation) deselects them entirely; (2) `tests/conftest.py` separately skip-gates every `gpu`-marked test whenever the cached preflight state is not `ppstructurev3_init_succeeded`, which covers CPU-only hosts, hosts without Paddle installed, hosts where GPU-bind fails, and hosts where the preflight import crashes (FR-001 / FR-019). Activation requires BOTH conditions to permit execution — `-m` not excluding gpu AND preflight resolving to `ppstructurev3_init_succeeded`. When the test does execute, the body raises `pytest.fail` so the deferred-implementation state surfaces loudly until the real assertion is wired in the GPU follow-up. The prior polish `f695a96` dropped the redundant `@pytest.mark.skip` decorator that blocked auto-activation even when both gates would have allowed it.
