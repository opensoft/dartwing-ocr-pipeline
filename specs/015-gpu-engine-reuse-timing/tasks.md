...
Single Python project. Source under `src/ledgerlinc_ocr/`, tests under `tests/`.
...
**Independent Test**: `python -m ledgerlinc_ocr.preprocessing --document-folder tests/stage1_vendor_identity/inv_001_easy --preprocess-profile ppstructurev3@gpu` — assert PPStructureV3 constructor invoked exactly once, `preprocess_output.json` validates against contract set 1.2.0, `pipeline_version` ends in `.gpu0`, and the final stdout `run_summary` line has the expected `phase_timings` keys.
...
- [ ] T040 Verify `pyproject.toml` is unchanged (no new pinned dependencies — see plan.md Technical Context). If feature work has accidentally introduced a dependency, justify or back out.
